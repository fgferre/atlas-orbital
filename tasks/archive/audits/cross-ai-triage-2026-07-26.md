...
| **F-03**  | Eclipse geometry is computed in didactically-compressed world units. `d_sun/d_moon` collapses 389 → 5.03; the Moon's angular radius is 10.1× too large; the penumbral spot covers ~87% of Earth's radius instead of ~46%; a shadow fires on ~86% of new moons against a real ~8.6%. Realistic mode is exact | `Planet.tsx:376`, `:384-387`; `astrophysics.ts:813-821`, `:668-669`. Realistic is exact because `AU_TO_3D_UNITS` and `KM_TO_3D_UNITS` share one linear factor (`astrophysics.ts:4-5`)                                                                                                                    | ext               |
...
| **D-02**  | The descent-brightening branch is unreachable: `minDistance = radius × 1.1` (`src/components/canvas/cameraNearPlane.ts:43`) against a 1.025 shell. It also kills the T5.1 ESun boost, since `camHeightGr` is always 0.1 against an `atmosphereHeight` of 0.025                                 | **Fix only the false comments.** Unlocking it buys a 7% band of camera travel whose destination is unshippable at current LOD                                                                                                                                                                                                         |
...
| BackSide atmosphere seen from below the surface    | `minDistance = focusRadius × 1.1` against a 1.025 shell stops the camera outside it, and surface mode's threshold is 2.5 radii from centre — above the surface, not below                                                                                                         |
...
- **The texture seam constant is derivable, not a knob.** `SphereGeometry` puts
  the u = 0.5 meridian on mesh +X, which is where node Q sits at W = 0, and
  increasing u drives z negative, which `ecliptic2ThreeJs` maps to increasing
  east longitude. Expected residual artist offset: zero. Verify empirically once
