...
- **Fix shipped** (`cd626dc`):
  ...
  5. Gates green: `npm run test:run` 873/873, `npm run lint` clean,
...
**T6.3-ε ship-row claim SUPERSEDED 2026-05-04** by first user-driven manual smoke that revealed the procedural mesh never renders visually at HYG positions. The infrastructure changes shipped at this commit (HygStellarMesh quality-strand guard + StarHoverPicker setSelectedId clear + GridProjectionLines HYG gate + spect coverage doc correction) remain valid; the rendered visual delivery does not. Recovery wave: see `tasks/waves/T6.4-visual-recovery.md`. Postmortem: `tasks/archive/postmortems/T6-visual-failure.md`. The original 2026-05-04 success-framing text was based on tests + lint + build + audits passing — none rendered GPU output, so the visual failure was undetectable until user manual smoke.
...
revealing T6 mesh invisibility). Estimate update **2026-05-06
night**: M1+M2 ✅, M2.5 S1-S7 + rounds 3 / 4 / 5 / 5b ✅,
**Round-6 (interactive flight) sub-tracks A through H ✅
shipped 2026-05-06** in 5 commits — port of Gaia's
`InteractiveCameraModule.go_to_object` flow combined with
`NaturalCamera.java`'s force/friction physics, plus integrator
sub-stepping hardening surfaced via the Round-6 e2e respec.
M2.5 close gate now flips to user-smoke Round-6 acceptance
(see STATUS.md). Remaining T6.4 scope: M3 (sprite ↔ mesh
cross-fade, ~2h, unblocked once M2.5 closes on smoke), M4
(class variation in stellar profile, ~6-8h), M5 (spect-missing
fallback, ~1-2h), M6 forward-port (~14h, 8 sub-tracks), M7
(final cleanup + smoke, ~30min). Codex audit per milestone
mandatory. Full plan, milestone status, audit history, scope
tags, acceptance criteria, and the round-3..5b + Round-6
narratives all live in the wave file linked above. This
ROADMAP entry is intentionally a pointer only (per L38
single-source-of-truth rule).

_Original 2026-05-05 inline plan removed in commit (this commit)
per Codex post-restructure audit — the `<details>` block was
duplicate-source-of-truth + git history + archive snapshot
already provide traceability. The wave file is canonical;
recover the inline draft via `git log -p tasks/ROADMAP.md` if
needed._

**Dependencies**: T6.0 + T6.1 + T6.2 + T6.3 (all shipped). T-Closeout +
T4.3 + T5.x re-bakes all blocked behind T6.4 visual delivery
(see wave file for the per-milestone acceptance criteria).
...
### T6 wave summary (status as of 2026-05-05; pre-2026-05-04 MVP claims superseded)

- **T6.0 → T6.3-ε infrastructure** (shipped over 17 commits):
  HYG focus extension, skipMask, StellarVisualProfile,
  spect-augmented binary, hysteresis gate, HygStellarMesh
  integration, click-fly-to wire, plus 7 Codex-caught surface
  bugs in T6.3-δ + T6.3-ε. Code is correct; visual delivery is
  not. **All pre-2026-05-04 success claims (any "MVP closed" /
  "live HYG zoom path" / "users can navigate HYG procedurally"
  framing) are SUPERSEDED** — first user-driven manual smoke
  (2026-05-04) revealed mesh never renders visually at parsec
  scale. See
  `tasks/archive/postmortems/T6-visual-failure.md`.
- **Active recovery (T6.4)**: see canonical wave file
  `tasks/waves/T6.4-visual-recovery.md`. Estimate, milestone
  list, scope tags, and acceptance criteria all live there.
  This summary is intentionally claim-free re T6.4 to avoid
  drift.
- **Polish path (was T6.4/T6.5 pre-2026-05-05, now T6.5/T6.6)**:
  remains optional polish post-T6.4 visual recovery.
