...
Active recovery wave (post 2026-05-04 user smoke revealed T6
mesh invisibility). M1-M7 all agent-side ✅ 2026-05-07; wave
acceptance now blocks on user smoke (see STATUS.md hot path
§"Higher-priority parallel work for the user" for the smoke
cycle spec). Per L38 single-source rule, all milestone status,
scope tags, audit narratives, and acceptance criteria live in
the wave file — this ROADMAP entry is intentionally a pointer.
Recover the prior 2026-05-06 inline detail via
`git log -p tasks/ROADMAP.md` if needed.

**Dependencies**: T6.0-T6.3 (all shipped). T-Closeout + T4.3 +
T5.x re-bakes all blocked behind T6.4 acceptance.
