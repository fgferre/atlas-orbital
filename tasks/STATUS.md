...
_Last updated: 2026-04-23 after **T4.4a gridrec math-extraction ship (`49fdaf0`)** — pure-TS mirror of `gridrec.fragment.glsl` constants + 9 helper functions landed as a building-block for the coming shader port (T4.4b). 15 numeric literals pinned byte-for-byte (N=10, BASE_LINE_WIDTH=5, BASE_COL_DIAG=(1,0.492,0.09,0.3), circle/square level frequencies 10/1 and 400/40, cross/diag pow exponents 2/3, etc); 9 pure helpers (rotateUV, smoothstep, radial alpha, heightScale fade, line width, circle/square grid funcs, center dampen, cull test, style→elevationMultiplier mapping) with 59 hand-derived sample values. Step-3 PRE-CHECK fixed a smaller ROADMAP drift (mentioned a `gridrec.vertex.glsl` that doesn't exist — Gaia pairs the fragment with `shader/default.vertex.glsl` per `RenderAssets.java:211`) + confirmed CIRCULAR is Gaia's default render path per `config.yaml:384`. DIFF GATE + independent SUBAGENT VERIFY (Sonnet) both PASS 15/15 constants + 9/9 helpers. Prior T2.5 + T2.6 lighting-drift ship (`9910eeb`) closed the residual drifts T2.4 did not cover. `envMapIntensity` 1.9-2.1 → 0.0 across all 5 presets (Gaia-exact: `pbr.fragment.glsl:620-621` uses the reflection skybox only for specular `finalReflection`, never for diffuse IBL; atlas's `scene.environmentIntensity` lift was the partial reason T2.4's ambient=0 didn't fully black-out dark sides). `shadowIntensity` 1.3-1.5 → 0.4 across all 5 presets (ROADMAP Option 1 empirical floor: drops focused-body over-brightness vs Gaia from ≈2.5× to ≈1.4× while keeping DirectionalLight shadow-map crater/cloud contrast legible; residual 40% drift is Three.js architectural — libGDX point-light shadow maps have no direct r3f equivalent). JSDoc header at `visualPresets.ts:33-49` documents the new Gaia-fidelity baselines with source-line citations; CLOSE_FLYBY's shadow/envMap per-preset differentiation retired (both were rooted in the now-deleted atlas-opinion lifts). Previously, T2.4 brought ambient, sun, bloom, and tone mapping into line — audit-surfaced shadow + IBL drifts are now closed as the final step of that wave. L32 captured earlier: visual-diff baseline PNGs need an explicit review gate when re-baked. Prior Codex audit triage + T2.4 notes remain below in history._
...
## → Next up: **T4.4b — gridrec fragment shader port + quad mount + EclipticGrid predecessor sweep** (the "real" T4.4 iteration, 2-3 d). T4.4a (`49fdaf0`) landed the TS math mirror; T4.4b ports the composite GLSL (`circle()`, `square()`, `circle_rec()`, `square_rec()`, `main()`) onto a Three.js shader material, mounts it on a camera/focus-centered quad replacing `EclipticGrid.tsx`, and sweeps the atlas predecessor under `feedback_no_effect_stacking.md`. T4.4c follows with the orientation toggle UI (Equatorial / Ecliptic / Galactic transform matrix swap per `GridRecursiveRadio.java:34-44`) + per-frame drivers for `u_tessQuality` / `u_heightScale` from Gaia's `getGridScaling(body.distToCamera, ...)`. Other Tier-4 candidates still valid and ordered by effort/value: **T4.2 camera cinematics** (1-2w, NaturalCamera damping + surface mode — cinematic-feel gap), **T4.5 MSDF text / constellations** (1-2w), **T4.1 camera-relative rendering** (2-3w, jitter fix), **T4.3 particle systems** (2-3w, ASTEROID BELT + KUIPER + MW particles — subsumes the old T4.7 panorama gap). T2.3b still BLOCKED (user asset delivery); T3.7 / T3.8 / T3.9 / T4.6 / T4.7 / T4.8 all shipped or closed; T2.0 / T2.1 / T2.3a / T2.4 / T2.5 / T2.6 / T3.3 / T3.4 / T3.5 / T3.6 all shipped. T2.2 remains demoted as opt-in (PSEUDO variant, non-default Gaia path).
