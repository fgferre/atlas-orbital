...
## → Next up: **T4.4e — projection lines** (~1 d). Final sub-wave to close T4.4 fully 1:1 with Gaia. When `origin=REFSYS` + camera focus active, Gaia draws connector lines between camera and focused body projected onto the grid — `GridRecUpdater.java:84-102` computes endpoints via `getCFPos` + `getZXLine` (horizontal) + `getYLine` (vertical). Ship shape: (1) atlas state slice `gridProjectionLines: boolean` (default `true` mirroring Gaia's `config.yaml:381 projectionLines: true`); (2) compute 4 line segments per frame from camera.position + focused-body world position, project onto the active grid orientation (transform via the inverse of the orientation matrix so the lines land on the tilted plane); (3) render via drei `<Line>` (already used by `PlanetOrbitLine` — T4.6 SDF patch applies). After T4.4e, the T4.4 wave is fully closed. Other Tier-4 candidates still valid and ordered by effort/value: **T4.2 camera cinematics** (1-2w, NaturalCamera damping + surface mode — cinematic-feel gap), **T4.5 MSDF text / constellations** (1-2w), **T4.1 camera-relative rendering** (2-3w, jitter fix), **T4.3 particle systems** (2-3w, ASTEROID BELT + KUIPER + MW particles — subsumes the old T4.7 panorama gap). T2.3b still BLOCKED (user asset delivery); T3.7 / T3.8 / T3.9 / T4.6 / T4.7 / T4.8 all shipped or closed; T2.0 / T2.1 / T2.3a / T2.4 / T2.5 / T2.6 / T3.3 / T3.4 / T3.5 / T3.6 all shipped. T2.2 remains demoted as opt-in (PSEUDO variant, non-default Gaia path).
