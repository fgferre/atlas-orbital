...
**This session (2026-04-23) shipped 11 feats + 3 doc commits + associated docs**:
...
## → Next up: **T4.2 wave (camera cinematics)**. T4.5 fully closed (α + β + δ shipped; γ retired). T4.9a' shipped with placeholder asset; T4.9b' retired; T2.3b deferred. T4.4 fully closed. Clear ship sequence:

1. **T4.2-α** — Proximity-aware damping (~2 d). Pure-TS port of `NaturalCamera.java:993-997` `counterAmount` curve as `src/lib/camera/proximityDamping.ts`. Replace OrbitControls' fixed `dampingFactor=0.05` with per-frame setter. Independent. PRE-CHECK: confirm three-stdlib OrbitControls allows live-mutating `dampingFactor`.
2. **T4.2-γ** — Inertial zoom physics (~3-5 d). Replace `NormalizedWheelZoom` step accumulator with velocity-integrating zoom mirroring `NaturalCamera.java:980-1010`. Independent of α/β.
3. **T4.2-β** — Surface mode (~3-4 d). Port `NaturalCamera.java:524-548` `surfaceModeFlag` (`distFromFocus < radius × 2.5 / fovFactor`) + free-rotation handler swap. Depends on T4.2-α.
