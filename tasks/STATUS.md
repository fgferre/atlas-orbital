...
## → Next up: **T4.4d — gridrec orientation toggle** (1 d). T4.4c (`2e42b8c`) closed the dynamic-driver gap; T4.4d mirrors `GridRecursiveRadio.java:34-48`'s `transform.setTransformName` swap between `null` (Equatorial) / `"eclipticToEquatorial"` (Ecliptic — atlas's current implicit default) / `"galacticToEquatorial"` (Galactic). Ship shape: (1) atlas state slice for `gridOrientation: "equatorial" | "ecliptic" | "galactic"` replacing or supplementing `showEclipticGrid`; (2) rotate `GridRecursive.tsx`'s mesh by the ecliptic→equatorial (obliquity 23.4393°) or galactic→equatorial (J2000 IAU matrix) rotation based on state; (3) UI toggle in `LayersPanel.tsx` between the three orientations with per-orientation color callouts (`gr.ccEq` / `ccEcl` / `ccGal` in `GridRecursiveRadio.java:37-47`). After T4.4d, T4.4e (projection lines when `origin=REFSYS` + camera focus active, per `GridRecUpdater.java:84-102`, ~1 d) is all that remains to close T4.4 fully 1:1 with Gaia. Other Tier-4 candidates still valid and ordered by effort/value: **T4.2 camera cinematics** (1-2w, NaturalCamera damping + surface mode — cinematic-feel gap), **T4.5 MSDF text / constellations** (1-2w), **T4.1 camera-relative rendering** (2-3w, jitter fix), **T4.3 particle systems** (2-3w, ASTEROID BELT + KUIPER + MW particles — subsumes the old T4.7 panorama gap). T2.3b still BLOCKED (user asset delivery); T3.7 / T3.8 / T3.9 / T4.6 / T4.7 / T4.8 all shipped or closed; T2.0 / T2.1 / T2.3a / T2.4 / T2.5 / T2.6 / T3.3 / T3.4 / T3.5 / T3.6 all shipped. T2.2 remains demoted as opt-in (PSEUDO variant, non-default Gaia path).
