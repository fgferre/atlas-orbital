...
**This session (2026-04-23) shipped 13 feats + 3 doc commits + associated docs**:
...
- **T4.2-γ shipped** (`032cba9`): inertial zoom physics. `src/lib/camera/zoomPhysics.ts` adds a 1D velocity buffer + closed-form exponential friction; `Scene.tsx:NormalizedWheelZoom` refactored from snappy step-accumulator to wheel-impulse → velocity → per-frame `dollyIn`/`dollyOut` with coast-down. Only T4.2-β (surface mode) remaining to close the wave.
...
## → Next up: **T4.2 wave continues — α + γ shipped (`dae3815` + `032cba9`)**, β next. T4.5 fully closed (α + β + δ shipped; γ retired). T4.9a' shipped with placeholder asset; T4.9b' retired; T2.3b deferred. T4.4 fully closed.

1. **T4.2-β** — Surface mode (~3-4 d). Port `NaturalCamera.java:524-548` `surfaceModeFlag` (`distFromFocus < radius × 2.5 / fovFactor`) + free-rotation handler swap (`NaturalCamera.java:529-535`). New `src/lib/camera/surfaceMode.ts` extracts the boolean rule + the "should reorient focus" sub-flag; `CameraController` toggles `OrbitControls`'s target-update behavior per frame based on the flag. Depends on T4.2-α (shipped) — surface-mode rotation smoothness rides on the proximity-aware damping curve. After T4.2-β: T4.2 wave fully closed.
