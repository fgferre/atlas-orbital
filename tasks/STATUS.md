...
| Fallback-frame | **P1** | **Eighteen analytical satellites render their orbit in the wrong plane outside the 2020-2030 validity window.** `engine.ts` drops to the Kepler fallback there and reads `body.orbit`, which `keplerProvider` **declares** to be ecliptic J2000 (`outputFrame`) and which for these records is not — while `satelliteUsesParentEquatorialFrame` is registry-driven and **date-blind**, so nothing rotates them any more. Measured plane shift 2025 → 2035: **Miranda 104.6°**, Ariel/Umbriel/Titania/Oberon ~98°, Iapetus 30.4°, the other Saturnians ~28°, Phobos 26.3°, Deimos 24.1°, the Galileans ~2°. Reachable by dragging the date slider — the wave's own smoke list says "drag Ceres to 1890". Invisible to the suite, whose satellite fixtures are all in-window. **The cause is heterogeneous, so do not fix it by pattern-match:** most carry parent-EQUATORIAL `i` (Tethys 1.12 against a true ecliptic 27.14), **13 of the 18 additionally carry a fabricated `O: 0, w: 0, M0: 0`**, and for **Iapetus the fabricated node is the whole story** — its `i: 15.47` is near its true ecliptic 17.03, and 2×15.5 ≈ the 30.4° measured. Five Saturnians (Mimas, Enceladus, Tethys, Dione, Rhea) carry real published phase and are frame-only. **The fix is known and mechanical** — W6 applied it to Charon and Triton (now 0.00°, window-edge jump = true orbital motion): put the body's own fixture-derived ecliptic elements in `body.orbit`, re-referenced to J2000 as `M0 − n·(epochJD − J2000)` **reusing its calibrated `nDegPerDay` for both the re-reference and `n`** — that identity is what makes the edge seamless. Frozen-plane residual afterwards is bounded at ~2× tilt-to-Laplace (≤~3° for most, ~8.5° Miranda) against 24-105° today. **Two things to settle first:** (a) the Sidebar `e` cell would swap published means for one-instant osculating values — Tethys 0.0001 → 0.000841, Ariel 0.0012 → 0.000328 — a _silent_ change to a displayed number, so disclose or decouple it rather than slipping it in; (b) `astrophysics.ts:1034/:1068/:1134` default `orbitParams = body.orbit` and look dead (`integration.ts:18` says the family was replaced) — confirm before the swap, not after. `celestialBodies.test.ts:144`'s Kepler-III gate is compatible (≤~0.1%). Found by an external audit of W6 on 2026-07-27. Deliberately NOT swept into W6: it predates the wave and touches 18 records. |
