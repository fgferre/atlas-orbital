...
_Last updated: 2026-04-22 after **T4.6 shipped** (`a6a3644`) — Gaia's quad-strip line SDF feathering (`line.quad.cpu.fragment.glsl:20-33`) ported as an `onBeforeCompile` patch on drei's `LineMaterial`. Orbit lines now have Gaia's `core = min(cos(π·x/2), 1-|x|)` envelope + `pow(core, 1.8)` alpha + `pow(core, 10)` bright-core stripe. 14 pinned math tests; DIFF GATE + SUBAGENT VERIFY 10/10 PASS; runtime smoke at 56 FPS zero console errors. First onda to use the new step-3 PRE-CHECK codified last commit. Session totals (11 commits): T2.0, T2.3a, T2.1, T3.3, T4.6 shipped; T3.8 doc ship; T3.7/T3.9/T4.7/T4.8 hygiene passes; L30+L31 lessons; 13-step Ship protocol enforced._
...
| **T4.6 — Quad-SDF line rendering**          | `a6a3644`                                                                    | **1:1 VERIFIED** (Step-3 PRE-CHECK + DIFF GATE + SUBAGENT VERIFY 10/10 PASS + runtime smoke). `line.quad.cpu.fragment.glsl:20-33` SDF feathering (`core = min(cos(π·x/2), 1-                                                                                                                                                                                                                                                                               | x                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | )`+`pow(core,1.8)`alpha +`pow(core,10)`bright-core) ported as`onBeforeCompile`patch on drei's LineMaterial.`lineSdfMath.ts`+ 14 pinned tests;`useGaiaSdfLinePatch.ts` preserves LineMaterial's original onBeforeCompile (USE_LINE_COLOR_ALPHA define) via chained bind. | Divergences documented: `(v_uv.y-0.5)*2.0` skipped (drei's vUv.y already [-1,1]); `layerBuffer` → `gl_FragColor`; `logarithmicDepth()` handled by Three's `<logdepthbuf_fragment>` + renderer-level flag; shader-side premultiply collapses to standard GPU alpha blend (algebraically equivalent). Applied only to `PlanetOrbitLine`; other drei `<Line>` usages (e.g. if any added later) get the plain LineMaterial unchanged unless they also install the hook. |
