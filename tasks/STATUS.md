...
_Last updated: 2026-04-22 after **T2.1 shipped** (`a2c6594`) — Gaia's default COMPLEX lens-flare (`lensflare.frag.glsl` complex branch) ported as new `LensFlareEffect` + Sun-projection driver (`ndcToLensFlareUv`) + solid-angle alpha linstep helper. Atlas default flipped from PSEUDO → COMPLEX, matching `config.yaml:606`. θ.4 PseudoLensFlareEffect stays importable for opt-in. 13 new pinned math tests; DIFF GATE + SUBAGENT VERIFY 10/10 PASS; runtime smoke confirmed COMPLEX ring pattern renders around the Sun at 58.5 FPS with zero console errors. Lens Closure Wave remaining: T2.3b (asset hot-swap, blocks on user CC-BY-4.0 delivery) + optional T2.2 (opt-in PSEUDO blur)._
...
## → Next up: Lens Closure Wave — **T2.3b CC-BY-4.0 asset hot-swap** (BLOCKS on user AI-gen delivery)

T2.0 ✅ `cd626dc`, T2.3a ✅ `51750c3`, T2.1 ✅ `a2c6594`.
Remaining: **T2.3b** (hot-swap placeholders once user delivers
CC-BY-4.0 replacements) + optional **T2.2** (opt-in PSEUDO
35-pass blur, not on the Gaia default path).
