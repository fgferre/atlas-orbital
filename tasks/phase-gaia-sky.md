...
| ✅ SHIPPED  | θ.1b | M      | Star vertex     | 2026-04-20 → 2026-04-21 — `22349b0`, `583268e`, `07606be`, `0131af0`, `54e14ca`, `f8d8bff`, `8668b20`, `9b13f18`, `0961591`, `fa23a27`. Vertex solid-angle + pseudo-size + billboard-quad + color pipeline + fragment saturate + LEN0 unit fix. |
...
| ✅ SHIPPED  | θ.4  | M-L    | Composer        | 2026-04-21 — `db407dc`. Pseudo lens flare + lensdirt + starburst spikes (merged pmndrs Effect: bias + pseudolens + lensdirt). HDR HalfFloat composer prereq wired. Blur chain omitted, `flareIntensity = 0.03` compensates.                     |
| → **NEXT**  | θ.6  | S      | Composer        | Grading finishes (direct-port CA/vignette/grain). Small, display-referred, easy to baseline.                                                                                                                                                    |
| pending #2  | θ.9  | M      | Scene-graph     | Orbit-lines quad-strip + core-glow shader. Base for θ.10 constellation reuse.                                                                                                                                                                   |
| pending #3  | θ.10 | M      | Scene-graph     | Constellations lines + first troika-MSDF labels. Shader reuse from θ.9.                                                                                                                                                                         |
| pending #4  | θ.12 | S      | Scene-graph     | Named star labels via troika (MSDF approx of Gaia Sky SDF — §12 notes).                                                                                                                                                                         |
| pending #5  | θ.8  | M      | Camera          | Camera feel (cinematic damping + FoV easing + surfaceMode). No shaders — ships before composer passes to isolate blame.                                                                                                                         |
| pending #6  | θ.1c | M      | Star vertex     | Billboard motion-trail stretch. **Deprioritized 2026-04-21** — user preferred lens effects (θ.4 shipped ahead) over motion trails. Depends on θ.1b (✅). RM hard-off.                                                                           |
| pending #7  | θ.5  | M      | Composer+Depth  | Camera motion blur. Depth/velocity buffer.                                                                                                                                                                                                      |
| pending #8  | θ.15 | M      | Composer        | NFAA + FXAA + LumaSharpen (direct ports, no SMAA). Slot near end so re-baseline happens once.                                                                                                                                                   |
| pending #9  | θ.14 | S      | Star vertex     | Variability twinkle. **Hard dep on θ.1b** (solid-angle axis — ✅ shipped). Size-multiplier per-star.                                                                                                                                            |
| pending #10 | θ.11 | M-H    | Backdrop/assets | Milky Way cubemap + dust. Asset pipeline + 2-layer blend; highest bloom-regression risk.                                                                                                                                                        |
| pending #11 | θ.7a | M      | Hero-LOD        | Detector de aproximação + corona billboard (hero-star).                                                                                                                                                                                         |
| pending #12 | θ.7b | L      | Hero-LOD        | Procedural surface (with literal FBM opts from θ-audit) + cross-fade. Largest item; slot last.                                                                                                                                                  |
